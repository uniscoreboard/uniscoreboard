...
fetch('scores-2023.json')
  ...
  .then(data => {
    ...
    playoffsHeader.textContent = '2023-24 NFL PLAYOFFS';
...
function appendGame(container, game, teamRecords) {
  ...
  const scoreText = `${game.AwayTeamScore}-${game.HomeTeamScore}`;
  ...
  logosDiv.innerHTML = `
    <div style="text-align: center; margin-right: 10px;">
      ...
      <div class="record">${teamRecords[awayTeam].wins}-${teamRecords[awayTeam].losses}</div>
    </div>
    <div style="padding-bottom: 0.5rem;">${scoreText}</div>
    <div style="text-align: center; margin-left: 10px;">
      <img src="${homeImg}" alt="${homeTeam} logo"><br>
      <div class="record">${teamRecords[homeTeam].wins}-${teamRecords[homeTeam].losses}</div>
    </div>
  `;
...
// Show only games and BYE for a specific team
...
// Show all games (but hide all BYEs)
function showAll() {
  const container = document.getElementById('scoresgrid');
  container.classList.remove('single-game');
